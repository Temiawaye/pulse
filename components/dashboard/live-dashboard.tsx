...
export function LiveDashboard({ metrics, chart, recent: initialRecent, uptime, projectId }: { metrics: Metrics; chart: ChartPoint[]; recent: MonitoringEvent[]; uptime: number | null; projectId?: string }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`events:${projectId ?? "all"}`).on("postgres_changes", filter, (payload) => { 
      if (!active) return; 
      setRecent((current) => mergeUniqueEvents(current, [payload.new as MonitoringEvent], 20)); 
      setUpdated(new Date()); 
      reconcile(); 
    }).subscribe((status) => { 
      if (!active) return; 
    ...
    <>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric 
          icon={Gauge} 
          label="Uptime" 
          value={uptime === null ? "Not configured" : `${uptime.toFixed(2)}%`} 
          small={uptime === null} 
        />
      </div>
      ...
      <Card className="mt-4 p-4 md:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="text-right">
            <span className="inline-flex items-center gap-1.5 text-xs capitalize text-[var(--muted)]">
              <Radio className={connection === "live" ? "h-3.5 w-3.5 text-[var(--accent)]" : "h-3.5 w-3.5 text-[var(--warning)]"} />
              {connection}
            </span>
        ...
        {chart.length ? 
          <MonitoringChart data={chart} /> 
          : <EmptyState 
            title="No monitoring events yet" 
            description="Send your first server-side event to populate live analytics." />}
      </Card>
      <Card className="mt-4 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[650px] text-left text-sm">
            ...
            <tbody>
              {recent.map((event) => 
              (
                <tr key={event.id} className="border-t motion-safe:animate-[fade-in_.2s_ease-out]">
                  <td className="px-5 py-3 font-mono text-xs">
                    {event.method}
                  </td>      
                  <td className="max-w-72 truncate px-3 py-3 font-mono text-xs">
                    {event.path}
                  </td>
        ...
        : <EmptyState 
            ...
            description="Incoming events will appear here without a refresh." 
          />
        }
      </Card>
    </>
...
function Metric({ icon: Icon, label, value, small }: { icon: typeof Activity; label: string; value: string; small?: boolean }) {
   return (
   <Card className="p-5">
