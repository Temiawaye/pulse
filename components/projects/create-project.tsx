    ...
    {state.apiKey ? 
      <div className="mt-6 space-y-5">
        <p className="text-sm text-[var(--accent-strong)]">
          Project created. Store this key now; Pulse will not show it again.
        </p>
        <SecretRow 
          label="Project ID" 
          value={state.projectId!} 
          onCopy={() => copy(state.projectId!, "id")} 
          copied={copied === "id"} 
        />
        <SecretRow 
          label="Ingestion API key" 
          value={state.apiKey} 
          onCopy={() => copy(state.apiKey!, "key")} 
          copied={copied === "key"} 
        />
        <pre className="overflow-x-auto rounded-md bg-[var(--background)] p-4 text-xs">
          <code>
            {`curl -X POST $PULSE_URL/api/events \\\n  -H "Authorization: Bearer $PULSE_API_KEY" \\\n  -H "Content-Type: application/json" \\\n  -d '{"method":"GET","path":"/health","statusCode":200,"responseTime":42}'`}
          </code>
        </pre>
        <Button 
          className="w-full" 
...
function SecretRow({ label, value, onCopy, copied }: { label: string; value: string; onCopy: () => void; copied: boolean }) {
  ...
  return (
    <div>
      ...
      <div className="mt-1 flex items-center gap-2 rounded-md border bg-[var(--background)] p-2">
